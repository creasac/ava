import { EventEmitter, CustomEvent } from './EventEmitter.js?v=1';
...
export class PCMPlayerWorklet extends EventEmitter {
  ...
  async initialize() {
    ...
    try {
      ...
      const processorCode = `
        class PCMProcessor extends AudioWorkletProcessor {
          constructor() {
            ...
            // Stats reporting
            this.frameCount = 0;
            this.reportInterval = 16;

            this.port.onmessage = (e) => {
              switch(e.data.type) {
                case 'audio':
                  if (e.data.sessionId === this.sessionId) this.addAudio(e.data.data);
                  break;
                case 'reset':
                  this.reset(e.data.sessionId);
                  break;
                case 'stream-ended':
      ...
      this.workletNode.port.onmessage = (e) => {
        if (e.data.sessionId !== this.playbackSession) return;
        switch (e.data.type) {
          case 'capacity':
          ...
          case 'position':
            this.metrics.samplesPlayed = e.data.samplesPlayed;
            this.emit('position', { samplesPlayed: e.data.samplesPlayed });
            break;
  ...
  processPendingChunks() {
    ...
    if (chunk.length <= this.availableCapacity) {
      ...
      this.workletNode.port.postMessage({
        type: 'audio',
        sessionId: this.playbackSession,
        data: chunk
      });
  ...
  set volume(value) {
    ...
    const now = this.audioContext.currentTime;
    this.gainNode.gain.cancelScheduledValues(now);
    this.gainNode.gain.setValueAtTime(clampedValue, now);
